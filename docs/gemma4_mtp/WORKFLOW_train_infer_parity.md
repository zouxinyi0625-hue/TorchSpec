...
### ★ max-concurrency=32 + spec_tokens=5（layer1）— 吞吐终于涨了 +6.5%

| conc=32 spec=5 | baseline (官方) | trained layer1 | Δ |
|--------|:---:|:---:|:---:|
| accept % | 65.75 | **76.64** | +10.9 |
| accept_len | 4.29 | **4.83** | +0.54 |
| **TPOT ms**（↓好） | 30.07 | **25.78** | **−14%** |
| **out tok/s** | 1058.4 | **1127.1** | **+6.5% ✅** |
| duration s | 299.82 | **279.90** | −20s |
| pos0 % | 86.20 | **90.56** | +4.4 |
| pos4 % | 47.02 | **63.52** | +16.5 |

**这是唯一吞吐真涨的场景，也验证了前面的机制**：conc=32（不饱和，GPU 有闲置算力）**且** spec=5（accept_len 有多 token 收益空间）**两个条件同时满足时**，更好的 draft 才把 accept↑ 变现成吞吐（+6.5%）+ 延迟改善（TPOT −14%）+ 更短总时长。

对照前面失败场景，坐实结论：
- conc=989 spec=5：**饱和** → 无闲置算力，吞吐平（收益进 TPOT）
- conc=32 spec=1：**收益空间太小**（accept_len 上限=2）→ 吞吐平
- **conc=32 spec=5：不饱和 + 收益空间够 → 吞吐 +6.5% ✅**

→ **部署指引**：要吃到吞吐红利，须在 **非饱和并发**下用 **spec≥5**。生产选点应扫 concurrency × spec_tokens 找 draft 收益变现的最优区间。

### ★★ no-MTP 锚点（conc=989，纯自回归 26B）— 解开"投机 vs 不投机"的 +38%

之前缺的关键基线：**关掉投机的纯自回归 26B**。有了它才能算"投机 vs 不投机"的真实红利。

| conc=989 | no-MTP (纯自回归) | 官方 MTP | trained full MTP |
|---|:---:|:---:|:---:|
| **out tok/s** | **934.83** | 1295.9 | 1328.4 |
| vs no-MTP | — | **+38.6%** 🎯 | **+42.1%** |
| TPOT ms | 107.75 | 100.73 | 89.86 |
| duration s | 340.34 | 237.7 | 234.91 |

**这解开了"为什么 +MTP 涨 ~30% 但换 draft 没涨"的核心矛盾**：

| 对比 | 吞吐 Δ | 归属 |
|------|:---:|------|
| **不投机 → 官方 MTP** | **+38.6%** | 投机机制本身的大红利（即用户说的 ~30%）|
| **官方 MTP → trained MTP** | **+2.5%**（full, 989 饱和） | 换更好 draft 的增量（饱和场景小）|
| **官方 MTP → trained MTP** | **+6.5%**（layer1, conc=32 spec=5） | 换更好 draft 的增量（非饱和场景大）|

→ **高并发下投机机制仍给 +38%（并非"饱和无法变现"）；只是这份红利官方 draft 就已拿到，"换更好 draft"的增量在饱和时小（+2.5%）、非饱和时大（+6.5%）。** draft 训练的价值 = 在 +38% 基础上再叠加 2.5%~6.5% 吞吐 + 全场景 TPOT/accept 改善。
